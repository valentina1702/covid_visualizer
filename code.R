...
top_countries_by_total_cases <- confirmed_cases_by_country %>%
  group_by(country) %>%
  summarize(total_cases=max(cum_cases)) %>% top_n(7,total_cases)
...
ggplot(confirmed_cases_top7_outside_china,aes(date,cum_cases))+geom_line(group=country,color=country) +  ylab("Cumulative confirmed cases")
